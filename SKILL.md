...
4. **处理用户的回答**：
   - 如果遇到材料里本身的例题，且用户回答错误，请耐心纠正，并把这个知识点存入 `memory/ai-tutor-state.json` 当前课程的 `review_queue` 数组中复习用。
...
3. **判卷与纠错**：用户回答后，根据 Chunk 中的绝对正确知识来判断对错。
   ...
   - 如果错了：严格纠正，并把这个错题存入 `memory/ai-tutor-state.json` 当前课程的 `review_queue` 中，然后**将 `current_chunk` + 1** 继续抽查。

### 👁️ 监督与督导模式 (Supervision & Evaluation Mode)

针对自律性较弱的用户，基于 OpenClaw 的 Cron 机制触发。

**1. 课中防走神 (Ping)**
如果在讲课或复习互动期间，用户超过 **15分钟** 没有回复：
- **主动 Ping**："同学，刚才讲的那段能跟上吗？怎么没声音啦？"
- **3次熔断机制**：连续催促最多 3 次（即 45 分钟）。如果用户始终未出现，在 `memory/ai-tutor-state.json` 中将当天状态标记为 `"paused"`。

**2. 每日学情考核 (Daily Evaluation)**
当触发“结束今天的学习”或到达设定的晚间 Cron 定时任务时：
- **提取数据**：读取 `memory/ai-tutor-state.json` 中今日推进的 `current_chunk` 增量，以及 `review_queue` 中新增的错题记录。
- **生成报告**：
  > - **严师/良师判定**：像真实的补习班班主任一样，对用户今天的表现进行评价。
  > - **正向反馈**：如果推进了多个 Chunk 且正确率高，给予极高情绪价值的夸奖。
  > - **负向反馈**：如果 Chunk 推进为 0，或者连续触发了防走神熔断，严厉地指出其偷懒行为，并反问调整计划。
  > - **预告**：告知明天将从哪个 Chunk 继续，或明天需要重点复习哪个错题。
